...
class TwsbrEnv(gym.Env):
    ...
    def __init__(self,
        ...
        state_limit = np.array([180, 180, 180, 10.0, 10.0, 10.0, 1.0, 1.0, 1.0, 10.0, 10.0, 10.0 , 180, 180, 180, 10.0, 10.0, 10.0, 1.0, 1.0, 1.0, 10.0, 10.0, 10.0])
        ...
        self.obs_min = np.array([-180, -180, -180, -10, -10, -10, -1, -1, -1, -10, -10, -10, -180, -180, -180, -10, -10, -10, -1, -1, -1, -10, -10, -10])
        self.obs_max = np.array([180, 180, 180, 10, 10, 10, 1, 1, 1, 10, 10, 10, 180, 180, 180, 10, 10, 10, 1, 1, 1, 10, 10, 10])
    ...
    def _get_reward(self):
        ...
        if self.target_mode == "self-balancing":
            ...
            # V7 
            reward -= abs(error_position_x)**2
            reward -= abs(pitch)**2
            reward -= abs(state_action_power)**2
            reward += 0.3 if abs(abs(error_position_x) < 0.01) else -0.3
            reward += 0.3 if abs(abs(pitch) < 0.01) else -0.3
            reward += 0.3 if abs(abs(state_action_power) < 0.01) else -0.3
            #reward += 0.05 if abs(abs(state_action_power) < 0.01) else -0.1
