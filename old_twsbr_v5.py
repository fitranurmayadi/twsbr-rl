...
class TwsbrEnv(gym.Env):
    ...
    def _get_obs(self):
        ...
        obs = np.concatenate((self.last_state, obs_temp))
    ...
    def _get_reward(self):
        ...
        error_position = target_position - abs(x)
        ...
        reward += 0.1 if abs(omega_y) < abs(prev_omega_y) else -0.1
        reward += 0.1 if abs(x) < abs(prev_x) else -0.1
        reward += 0.1 if abs(x_dot) < abs(prev_x_dot) else -0.1
        reward += error_angle * 10 #error always negative
        reward += error_position * 10
